...
if os.path.exists('model/model.json'):
    ...
    classifier.add(Dense(output_dim = 2, activation = 'softmax')) #this output layer will predict 1 disease from given 21 disease images
    print(classifier.summary())
    classifier.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy']) #compile cnn model
    hist = classifier.fit(X_train, Y_train, batch_size=32, epochs=10, shuffle=True, verbose=2) #build covid detection CNN model with given Xtrain and Ytrain images
